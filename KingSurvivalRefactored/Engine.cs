...
namespace KingSurvival
{
    public class Engine
    {
        ...
        private bool ValidateCommand(string commandToCheck)
        {
            ...
            {
                foreach (var letterRepresentation in this.charRepresentationsPawns)
                {
                    if (commandToCheck == string.Format("{0}{1}", letterRepresentation, Direction.DL.ToString()) ||
                        commandToCheck == string.Format("{0}{1}", letterRepresentation, Direction.DR.ToString()))
                    {
                        commandIsValid = true;
                        break;
                    }
        ...
        {
            bool valid = this.gameBoard.IsInGameField(position) && this.gameBoard[position.Row, position.Col] == ' ';
            return valid;
        }

    ...
    public class GameBoard
    {
        ...
        public void DrawGameBoard()
        {
            ...
            for (int row = 0; row < this.gameBoard.GetLength(0); row++)
            {
                for (int col = 0; col < this.gameBoard.GetLength(1); col++)
                {
                    ...
                    bool isCellIn = this.IsInGameField(positionCell);
                    if (isCellIn)
                    {
                        if (row % 2 == 0)
                        {
        ...
        private void InitializeGameField(List<Figure> figures)
        {
            ...
            foreach (var figure in figures)
            {
                ...
                if (this.IsInGameField(figure.Position))
                {
                    this.gameBoard[figure.Position.Row, figure.Position.Col] = figure.Symbol;
                }
                else
                {
